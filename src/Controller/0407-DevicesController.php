        ...
        if ($session->read('_device_targetname') != ''){
            ...
            if ($this->request->is('post')) {
                ...
                if (!empty($devices)){
                    ...
                    $start_date = date('Y-m-d H:i:s', strtotime('-330 minutes', strtotime($start_date)));
                    $end_date = date('Y-m-d H:i:s', strtotime('-330 minutes', strtotime($end_date)));

                    // $from_date = date(DATE_ISO8601, strtotime($start_date));
                    // $to_date = date(DATE_ISO8601, strtotime($end_date));
        ...
        if (!empty($_device)){
            ...
            if ($t_login->responseCode == 200){
                ...
                //     debug($this->request->data['device_id']);
                    // $t_devices = $obj_traccar->positions($this->request->data['device_id']);//, $from_date, $to_date);
                // }
                // debug(empty(json_decode($t_devices->response)));
                if (empty(json_decode($t_devices->response))){
                    $t_devices = $obj_traccar->devices(('id='.$this->request->data['device_id']));
                    $position_array = json_decode($t_devices->response);
                    $device_info = json_decode($t_devices->response);
...
//            400 - duplicate identifier
    ...
    public function test(){
        ...
        debug($obj_traccar->devices('id=26'));
        $_device = json_decode($obj_traccar->devices('id=26')->response);
        // debug($_device);

        // $kmtoknots = number_format((float)(15 * 0.539957), 2, '.', '');
        $_attrib = '"speedLimit":"15.00"';

        debug($obj_traccar->addDeviceAttrib($_device[0]->id, $_device[0]->name, $_device[0]->uniqueId, $_attrib));
        // debug($obj_traccar->commandtypes(13));

        debug($obj_traccar->getDeviceCommands($_device[0]->id));
    }
