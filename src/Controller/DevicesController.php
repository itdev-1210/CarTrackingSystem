...
class DevicesController extends AppController
{
    ...
    public function singledevicetrack()
    {
        ...
        if ($session->read('_device_targetname') != '') {
            ...
            if (($session->read('_user_type') == 1) || ($session->read('_user_type') == 5)) {
                $_devices = $this->Devices->find('all')
                    ->select([
                        'DISTINCT Devices.id', 'Devices.device_traccarid',
    ...
    public function trackdevice($device_id = null)
    {
        ...
        if ($session->read('_device_targetname') != '') {
            ...
            if ($this->request->is('post')) {
                ...
                if (!empty($devices)) {
                    ...
                    $from_date = $start_date = date('Y-m-d H:i:s', strtotime('-330 minutes', strtotime($start_date)));
                    $to_date = $end_date = date('Y-m-d H:i:s', strtotime('-330 minutes', strtotime($end_date)));
                    ...
                    if ($t_login->responseCode == 200) {
                        ...
                        // $t_devices = json_decode($obj_traccar->lcpositions($device_id, $from_date, $to_date));
                        //
                        // foreach ($t_devices as $key => $value) {
                        //     $value->attributes = json_decode($value->attributes);
                        //     $device_init_info[] = $value;
                        // }
        ...
        $this->set(compact('device_id', 'devices', 'device_init_info', 'start_date', 'end_date'));
    }
    ...
    public function ajaxsetdevicetargetsetname()
    {
        ...
        if ($session->read('_user_type') == 1) {
            $devices = $this->Devices->find('all')
                ->select(['Devices.device_targetname'])
                ->where(['Devices.device_targetname LIKE "%' . $this->request->data['device_targetname'] . '%"'])
                ->andwhere(['Devices.device_status = 1'])
                ->andwhere(['Devices.created_by' => $session->read('_user_id')])
                ->count();
        } else {
    ...
    public function ajaxgetdeviceposition()
    {
        $session = $this->request->session();
        ...
        if (!empty($_device)) {
            ...
            if ($t_login->responseCode == 200) {
                ...
                $device_info = $obj_traccar->devices(('id=' . $this->request->data['device_id']));
                $device_info = json_decode($device_info->response);

                $t_devices = $obj_traccar->position($device_info[0]->positionId);
                ...
                if (!empty($position_array)) {
                    ...
                    $position_array[(count($position_array) - 1)]->speed = ($position_array[(count($position_array) - 1)]->speed * 1.852);
                    $position_array[(count($position_array) - 1)]->deviceTime = date('Y-m-d H:i:s', strtotime($position_array[(count($position_array) - 1)]->deviceTime));

                    $position_array[(count($position_array) - 1)]->fixTime = date('Y-m-d H:i:s', strtotime($position_array[(count($position_array) - 1)]->fixTime));

                    $position_array[(count($position_array) - 1)]->deviceStatus = $device_info[0]->status;
                    ...
                    if (!empty($position_array)) {
                        foreach ($position_array as $key => $value) {
                            if ($value->deviceId == $this->request->data['device_id']) {
                                // debug($obj_traccar->devices('id='.$this->request->data['device_id']));
                                $tmp_devices = json_decode($obj_traccar->devices('id=' . $this->request->data['device_id'])->response);
                                $position_array[$key]->deviceStatus = $tmp_devices[0]->status;
    ...
    public function edit($id = null)
    {
        ...
        if ($this->request->is(['patch', 'post', 'put'])) {
            ...
            if (($this->request->data['device_registrationdate_org'] == '') && ($this->request->data['device_status'] == 1)
            ) {
            ...
            if (trim($this->request->data['device_overspeed']) != '') {
                $_attrib = '"speedLimit":"' . (trim($this->request->data['device_overspeed']) * 0.539957) . '"';
            } else {
                $_attrib = '"speedLimit":"0"';
            }
            $t_updtdevice = $obj_traccar->editDevice($this->request->data['device_traccarid'], $this->request->data['device_targetname'], $this->request->data['device_identifier'], $_attrib);
